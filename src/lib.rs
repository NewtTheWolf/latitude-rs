...
impl Client {
    ...
    ///     key2: 42,
    /// };
    /// let response = client.run_document("path/to/document", parameters, false).await;
    /// ```
    pub async fn run_document<T>(
        &self,
        path: &str,
        parameters: T,
        stream: bool,
    ) -> Result<RunResponse, Box<dyn Error>>
    where
        T: Serialize,
    {
        let url = format!(
